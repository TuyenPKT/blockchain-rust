...
mempool_db/
  "tx:{txid}"                        → raw tx bytes (wire format)
  "fee:{u64::MAX-rate_msat:020}:{txid}" → ""   ← lex scan = highest fee first
  "ts:{txid}"                        → 16 bytes [ts_ns: u64 LE][fee_rate: u64 LE]
```
...
- [x] v17.2 — **Mempool Realtime**: `src/pkt_mempool_sync.rs` — `MempoolDb` (`~/.pkt/mempooldb`); key schema: `tx:{txid_hex}`→raw bytes, `fee:{u64::MAX-rate:020}:{txid_hex}`→"" (lex scan = highest fee first), `ts:{txid_hex}`→16 bytes [ts_ns u64][fee_rate u64]; `put_tx(txid, raw, fee_rate, ts_ns)` ghi 3 keys; `has_tx(txid)` O(log n) check; `evict_confirmed(txids)` đọc fee_rate từ ts: key → reconstruct fee: key → delete 3 keys O(log n)/tx; `get_pending(limit)` scan fee: prefix → highest fee first; `count()` scan tx: prefix; `fee_rate_histogram()` → 11 buckets [0,1,2,5,10,20,50,100,200,500,1000] msat/vB; `calc_fee_rate_msat(tx, size, utxo_db)` = (sum_inputs - sum_outputs) × 1000 / size; `sync_mempool(stream, magic, utxo_db, mempool_db)`: send `mempool` message → recv inv (tx items, timeout 5s) → send getdata → recv `Unknown{cmd="tx", payload}` → parse với `Cursor<&[u8]>` + public `read_tx_s` từ `pkt_block_sync` → store; `pkt_block_sync`: `read_tx_s` made pub; `BlockApplyResult` có thêm `confirmed_txids: Vec<[u8;32]>`; `sync_one_block` và `sync_blocks` thêm `mempool_db: Option<&MempoolDb>` → gọi `evict_confirmed` sau mỗi block; `pkt_sync::cmd_sync`: open mempooldb, Phase 4 mempool poll sau block sync, wipe mempooldb khi chain_reset; API: `GET /api/testnet/mempool?limit=` + `GET /api/testnet/mempool/fee-histogram` (PathState.open_mempool() per-request read_only) — **+14 tests** (2076 total)
