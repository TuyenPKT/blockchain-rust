...
**Version hiện tại: v5.0 ✅ — 63/63 tests pass, 0 errors, 0 warnings**
...
Stack đã có:
  ...
  v5.0       Performance (src/performance.rs):
               UtxoIndex: secondary addr_idx → O(1) balance_of / utxos_of
               BlockCache: hash_to_height HashMap → O(1) contains_hash
               fast_merkle: raw [u8;32] Merkle (no intermediate hex encode)
               fast_merkle_txids: drop-in for Block::merkle_root_txid callers

Next: v5.1 — Security hardening
```
