...
/// Validate a TokenTx against a registry: token exists + sender balance sufficient.
/// Returns `Ok(())` if valid, `Err(reason)` otherwise.
pub fn validate_token_tx(
    ttx: &TokenTx,
    registry: &crate::token::TokenRegistry,
) -> Result<(), String> {
    if registry.tokens.get(&ttx.token_id).is_none() {
        return Err(format!("token '{}' not found", ttx.token_id));
    }
    let balance = registry.balance_of(&ttx.token_id, &ttx.from);
    if balance < ttx.amount {
        return Err(format!(
            "insufficient balance for '{}': have {}, need {}",
            ttx.from, balance, ttx.amount
        ));
    }
    if ttx.amount == 0 {
...
mod tests {
    ...
    // ── v10.4 — validate_token_tx ─────────────────────────────────────────

    fn make_registry_with_balance() -> crate::token::TokenRegistry {
        let mut r = crate::token::TokenRegistry::new();
        r.create_token("TKN", "Test Token", "TKN", 18, 1_000, "alice").unwrap();
        r
    }

    #[test]
    fn test_validate_token_tx_valid() {
        let r = make_registry_with_balance();
        let ttx = TokenTx::new("TKN", "alice", "bob", 100);
        assert!(validate_token_tx(&ttx, &r).is_ok());
    }

    #[test]
    fn test_validate_token_tx_unknown_token() {
        let r = make_registry_with_balance();
        let ttx = TokenTx::new("UNKNOWN", "alice", "bob", 1);
        let err = validate_token_tx(&ttx, &r).unwrap_err();
        assert!(err.contains("not found"));
    }

    #[test]
    fn test_validate_token_tx_insufficient_balance() {
        let r = make_registry_with_balance();
        let ttx = TokenTx::new("TKN", "alice", "bob", 2_000); // alice only has 1_000
        let err = validate_token_tx(&ttx, &r).unwrap_err();
        assert!(err.contains("insufficient balance"));
    }

    #[test]
    fn test_validate_token_tx_zero_amount() {
        let r = make_registry_with_balance();
        let ttx = TokenTx::new("TKN", "alice", "bob", 0);
        let err = validate_token_tx(&ttx, &r).unwrap_err();
        assert!(err.contains("amount must be > 0"));
    }

    #[test]
    fn test_validate_token_tx_unknown_sender_has_zero_balance() {
        let r = make_registry_with_balance();
        let ttx = TokenTx::new("TKN", "stranger", "bob", 1);
        let err = validate_token_tx(&ttx, &r).unwrap_err();
        assert!(err.contains("insufficient balance"));
    }
}
