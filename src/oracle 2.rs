...
pub struct OracleReport {
    ...
    pub reporter:   String,    // oracle node address (compressed pubkey hex)
    pub round:      u64,       // round number
    /// v10.7: real ECDSA sig (compact 64 bytes → 128-char hex) when pubkey_hex is set;
    /// legacy blake3 hash (64-char hex) when pubkey_hex is empty.
    pub signature:  String,
    /// v10.7: compressed secp256k1 pubkey hex (33 bytes = 66 chars). Empty for legacy reports.
    pub pubkey_hex: String,
}

impl OracleReport {
    /// Legacy constructor — uses blake3 hash as signature (no private key needed).
    pub fn new(
        ...
        let signature = hex::encode(Self::signing_payload_raw(&feed_id, price, timestamp, &reporter, round));
        OracleReport { feed_id, price, timestamp, reporter, round, signature, pubkey_hex: String::new() }
    }

    /// v10.7 — Compute 32-byte signing payload (blake3 hash of report fields).
    fn signing_payload_raw(feed_id: &str, price: u64, timestamp: u64, reporter: &str, round: u64) -> [u8; 32] {
        let mut h = blake3::Hasher::new();
        h.update(b"oracle_report_v107");
    ...
    /// v10.7 — Create a report signed with a real secp256k1 ECDSA key.
    pub fn signed(
        feed_id: impl Into<String>,
        price: u64,
        timestamp: u64,
        ...
        let feed_id  = feed_id.into();
        let reporter = reporter.into();
        let payload  = Self::signing_payload_raw(&feed_id, price, timestamp, &reporter, round);
        let secp     = Secp256k1::new();
        let msg      = secp256k1::Message::from_slice(&payload).expect("32-byte payload");
    ...
    /// v10.7 — Verify: ECDSA if pubkey_hex is set, else legacy blake3 hash check.
    pub fn verify(&self) -> bool {
        if self.pubkey_hex.is_empty() {
            // Legacy: re-derive blake3 hash and compare
            let expected = hex::encode(Self::signing_payload_raw(
                &self.feed_id, self.price, self.timestamp, &self.reporter, self.round,
            ));
            return self.signature == expected;
        }
        // ECDSA path
        let payload  = Self::signing_payload_raw(&self.feed_id, self.price, self.timestamp, &self.reporter, self.round);
        let secp     = Secp256k1::new();
        let msg = match secp256k1::Message::from_slice(&payload) { Ok(m) => m, Err(_) => return false };
        let sig_bytes = match hex::decode(&self.signature) { Ok(b) => b, Err(_) => return false };
        let sig = match secp256k1::ecdsa::Signature::from_compact(&sig_bytes) { Ok(s) => s, Err(_) => return false };
        let pk_bytes  = match hex::decode(&self.pubkey_hex) { Ok(b) => b, Err(_) => return false };
        let pk = match PublicKey::from_slice(&pk_bytes) { Ok(k) => k, Err(_) => return false };
...
impl OracleFeed {
    ...
    /// `current_time`: unix timestamp now — used for staleness check.
    pub fn submit(&mut self, report: OracleReport, current_time: u64) -> Result<Option<RoundData>, String> {
        // v10.7 — Staleness check: reject if report timestamp is older than heartbeat
        if report.timestamp + self.heartbeat < current_time {
            return Err(format!(
                "Stale report: age={}s > heartbeat={}s",
                current_time.saturating_sub(report.timestamp), self.heartbeat
            ));
        }
        // v10.7 — ECDSA signature verification (or legacy blake3 for backward compat)
        if !report.verify() {
...
/// Off-chain oracle node — v10.7: generate ECDSA keypair, sign every report
pub struct OracleNode {
    /// Compressed pubkey hex — used as node identity/address
    pub address:    String,
    pub stake:      u64,
    pub reports:    u64,
    pub slashed:    u64,
...
impl OracleNode {
    /// Create a new oracle node — generates a fresh secp256k1 keypair.
    pub fn new(stake: u64) -> Self {
        let secp         = Secp256k1::new();
        let (sk, pk)     = secp.generate_keypair(&mut rand::thread_rng());
        let address      = hex::encode(pk.serialize());
        OracleNode { address, stake, reports: 0, slashed: 0, secret_key: sk, public_key: pk }
    }

    /// Tạo report ký bằng ECDSA (feed_id, price, timestamp, round đều được sign)
    pub fn report(
        &mut self,
        feed_id: &str,
        price: f64,
...
impl OracleRegistry {
    ...
    pub fn submit(&mut self, report: OracleReport, current_time: u64) -> Result<Option<RoundData>, String> {
        ...
        feed.submit(report, current_time)
    }
